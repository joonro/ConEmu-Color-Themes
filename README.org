...
- Use =Install-ConEmuTheme.ps1= PowerShell script. First, the script will always create
  ...
  1. To add a theme to your config file:
     ...
     #+END_SRC
     If a folder is specified instead of a theme file (e.g. =-ThemePathOrName themes=), then all the theme files under it are imported.
  2. To remove a theme from your config file:
     ...
     #+END_SRC
     If a theme file is specified instead of a theme name (e.g. =-ThemePathOrName themes\oceans16-dark.xml=), then the theme from the theme file is removed from the config. This allows you to remove a theme without having to look up its exact name.
